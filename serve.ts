...
//
// Auth API requests (/api/auth/*) are proxied to the standalone auth server on
// port 3001 to keep cookie handling outside of TanStack Start's server functions.
  ...
  try {
    Bun.serve({
      ...
      async fetch(req) {
        ...
        // Proxy /api/* to the standalone auth server
        if (url.pathname.startsWith("/api/")) {
          const upstream = `http://127.0.0.1:${AUTH_PORT}${url.pathname}${url.search}`;
          try {
